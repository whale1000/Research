...
class NeRFDataset:
    def __init__(self, opt, device, type='train', n_test=10):
        ...
        transform = np.load(os.path.join(self.opt.workspace, 'transform.npy'), allow_pickle=True).item()
        self.mode = 'colmap'
        # load image size
        ...
        self.near = 2.5#self.opt.min_near
        self.far = -2.5#1000 # infinite
        y = self.H / (2.0 * fl_y)
        ...
        self.mvps = self.projection.unsqueeze(0).to(float) @ torch.inverse(self.poses)
