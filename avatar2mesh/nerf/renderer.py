...
class NeRFRenderer(nn.Module):
    def __init__(self, opt):
        ...
        if self.individual_dim > 0:
            self.individual_codes = nn.Parameter(torch.randn(self.individual_num, self.individual_dim) * 0.1)  # 生成独立编码张量矩阵
        else:
    ...
    def export_stage1(self, path, h0=2048, w0=2048, png_compression_level=3):
        ...
        with open(mlp_file, 'w') as fp:
            json.dump(mlp, fp, indent=2)  # 导出mlp参数
    ...
    def export_stage0(self, save_path, resolution=None, decimate_target=1e5, dataset=None, S=128):
        ...
        if resolution == self.grid_size:  # 若分辨率等于grid_size
    ...
    def render(self, rays_o, rays_d, index=None, dt_gamma=0, bg_color=None, perturb=False, max_steps=1024, T_thresh=1e-4, cam_near_far=None, shading='full', **kwargs):
        ...
        # contiguous()用于创建一个新的张量，该张量具有与原始张量相同的数据，但具有满足连续内存要求的新内存布局。
        # .contiguous()连续张量是一种张量，其中元素存储在连续的内存块中，从而实现更快、更高效的计算。
        # 但是，某些操作可能会导致不连续的张量，这可能会减慢计算速度并导致某些函数出错。
        rays_o = rays_o.contiguous().view(-1, 3)  # 光线原点
        rays_d = rays_d.contiguous().view(-1, 3)  # 光线方向

        N = rays_o.shape[0]  # 光线数量
        device = rays_o.device

        # pre-calculate near far，预先计算远近边界
        nears, fars = raymarching.near_far_from_aabb(rays_o, rays_d, self.aabb_train if self.training else self.aabb_infer, self.min_near)
        if cam_near_far is not None:  # 如果相机远近边界不为None
        ...
        if self.individual_dim > 0:
            if self.training:
                ind_code = self.individual_codes[index]  # 根据index生成独立编码
        ...
        if self.training:  # 如果正在训练
            ...
            # plot_pointcloud(xyzs.reshape(-1, 3).detach().cpu().numpy())  # xyz可视化

            # ray-wise to point-wise，从逐个光线到逐个点
            ...
            dirs = safe_normalize(dirs)  # 采样点方向归一化
            with torch.cuda.amp.autocast(enabled=self.opt.fp16):
                sigmas, rgbs, speculars = self(xyzs, dirs, ind_code, shading)  # 输入采样点位置和方向，独立编码，颜色类型参数到NeRFNetwork以计算采样点的sigma、rgb、镜面颜色
            
            if self.opt.sdf:  # sdf设置
                # sigmas are sdf，采样点的密度作为sdf
                ...
                # 迭代系数iter_cos，这个系数是一个对true_cos的逐步逼近，其中使用了一个ReLU函数和一个余弦函数的组合。
                # 该系数最终用于调整当前时刻和下一个时刻之间的密度值，以便更准确地估计形状。
                ...
                p = prev_cdf - next_cdf  # 使用上一时刻的累积分布函数减去下一时刻的累积分布函数
                c = prev_cdf
        ...
        return results  # 返回渲染结果
    
    # phase 2
    def render_stage1(self, epoch, rays_o, rays_d, mvp, h0, w0, index=None, bg_color=None, shading='full', **kwargs):
        ...
        if epoch>50:
            ...
            vertices = self.vertices + self.vertices_offsets.mean(0) # [N, 3]，mesh网格平移位姿调整

        vertices_clip = torch.matmul(F.pad(vertices, pad=(0, 1), mode='constant', value=1.0).to(float), torch.transpose(mvp, 0, 1)).float().unsqueeze(0) # [1, N, 4]，顶点修剪
    ...
    @torch.no_grad()
    def update_triangles_errors(self, loss):  # 更新三角面片误差
        # loss: [H, W], detached!
        ...
        if TORCH_SCATTER is None:
            import torch_scatter  # whl文件下载地址https://pytorch-geometric.com/whl/
