...
class SNARFDeformer():
    ...
    def deform_test(self, pts, model):
        ...
        rgb_cano = torch.zeros_like(pts_cano_all).float()  # 标准空间采样点颜色
        sigma_cano = torch.zeros_like(pts_cano_all[..., 0]).float()  # 标准空间采样点体密度
        if valid.any():
            with torch.cuda.amp.autocast():
                rgb_cano[valid], sigma_cano[valid] = model(pts_cano_all[valid], None)  # 跳转到instant_avatar/models/networks【将采样点输入网络，返回对应颜色和体密度】
