...
from .networks.net import NeRFNGPNet
import torch
import numpy as np
...
from untils import get_c2w, get_mvp, get_stage1_opt, imgs_align, make_rays
import logging
...
class DNeRFModel(pl.LightningModule):
    def __init__(self, opt, datamodule) -> None:
        ...
        self.net_coarse = hydra.utils.instantiate(opt.network)  # 根据confs/network/ngp.yaml文件设置
        ...
        self.renderer = hydra.utils.instantiate(opt.renderer)  # 根据confs/renderer/raymarcher_acc.yaml文件设置
    ...
    def forward(self, batch, stage, eval_mode=False):
        ...
        return self.renderer(rays,
                             lambda x, _: self.deformer(x, self.net_coarse, eval_mode),  # 跳转到snarf_deformer.py中的__call__方法
                             self.net_coarse,  # 实际的完整网络
                             batch,
                             stage,
                             eval_mode=eval_mode,
                             noise=1 if use_noise else 0,
                             bg_color=batch.get("bg_color", None))  # 跳转到raymarcher_acc.py中的__call__方法

    @torch.no_grad()
    def render_image_fast(self, batch, img_size, stage):
        ...
        d = self.forward(batch, stage, eval_mode=True)
    ...
    def training_step(self, batch, *args, **kwargs):
        ...
        predicts = self.forward(batch, stage=0, eval_mode=False)
    ...
    def training_stage1(self, batch, img_size, stage):
        if hasattr(self, "SMPL_param") and self.opt.optimize_SMPL.get("is_refine", False):
            body_params = self.SMPL_param(batch["idx"])
            for k in ["global_orient", "body_pose", "transl"]:
                assert batch[k].shape == body_params[k].shape
        ...
        d = self.forward(batch, stage, eval_mode=False)
        rgb = d["rgb_coarse"].reshape(-1, *img_size, 3)
        depth = d["depth_coarse"].reshape(-1, *img_size)
        alpha = d["alpha_coarse"].reshape(-1, *img_size)
        return rgb, depth, alpha
        gt_rgb = cv2.imread(batch['index'], cv2.IMREAD_UNCHANGED)
        pred_rgb = (torch.cat([rgb, alpha[..., None]], dim=-1).detach().cpu().numpy() * 255).astype(np.uint8)[0]  # 合并像素rgb和alpha值  # 第一阶段预测的像素颜色
                    
        pred_rgb, gt_rgb = imgs_align(pred_rgb, gt_rgb)

        gt_rgb = torch.FloatTensor(gt_rgb)
        pred_rgb = torch.FloatTensor(pred_rgb)

        losses = {}
        loss_rgb = F.mse_loss(pred_rgb, gt_rgb, reduction="mean")
        losses["loss"] = loss_rgb
        ...
        if self.automatic_optimization:
            ...
            loss = losses["loss"]
            # optimizer = self.optimizers(False)
            optimizer, _ = self.configure_optimizers()
    ...
    def validation_step(self, batch, batch_idx, *args, **kwargs):
        ...
        rgb, depth, alpha, counter = self.render_image_fast(batch, img_size, stage=0)
        ...
        if batch_idx == 0:
            ...
            rgb_cano, *_ = self.render_image_fast(batch, img_size, stage=0)
