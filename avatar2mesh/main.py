...
if __name__ == '__main__':
    ...
    parser.add_argument('--workspace', type=str, default='workspace')  # 项目目录
    parser.add_argument('--seed', type=int, default=0)  # 随机种子
    parser.add_argument('--stage', type=int, default=0, help="training stage")  # 训练阶段
    parser.add_argument('--ckpt', type=str, default='latest')  # 检查点文件保存路径
    parser.add_argument('--fp16', action='store_true', help="use amp mixed precision training")
    parser.add_argument('--sdf', action='store_true', help="use sdf instead of density for nerf")  # 是否使用sdf替代nerf的密度
    parser.add_argument('--tcnn', action='store_true', help="use tcnn's gridencoder")  # 是否使用tcnn的网格编码

    ### testing options， 测试设置
    parser.add_argument('--test', action='store_true', help="test mode")  # 测试模式
    parser.add_argument('--test_no_video', action='store_true', help="test mode: do not save video")  # 是否保存video
    parser.add_argument('--test_no_mesh', action='store_true', help="test mode: do not save mesh")  # 是否保存mesh
    parser.add_argument('--camera_traj', type=str, default='', help="nerfstudio compatible json file for camera trajactory")

    ### dataset options
    parser.add_argument('--data_format', type=str, default='nerf', choices=['nerf', 'colmap', 'dtu'])  # 数据集格式
    parser.add_argument('--train_split', type=str, default='train', choices=['train', 'trainval', 'all'])  # 训练集划分
    parser.add_argument('--preload', action='store_true', help="preload all data into GPU, accelerate training but use more GPU memory")  # 预先加载所有数据到GPU以加速训练
    ...
    parser.add_argument('--downscale', type=int, default=1, help="downscale training images")  # 下采样缩放
    ...
    ### training options
    parser.add_argument('--iters', type=int, default=30000, help="training iters")  # 训练迭代次数
    parser.add_argument('--lr', type=float, default=1e-2, help="initial learning rate")  # 初始化学习率
    ...
    parser.add_argument('--cuda_ray', action='store_true', help="use CUDA raymarching instead of pytorch")  # 使用CUDA光线步进算法
    parser.add_argument('--max_steps', type=int, default=1024, help="max num steps sampled per ray (only valid when using --cuda_ray)")
    parser.add_argument('--update_extra_interval', type=int, default=16, help="iter interval to update extra status (only valid when using --cuda_ray)")
    parser.add_argument('--max_ray_batch', type=int, default=4096, help="batch size of rays at inference to avoid OOM (only valid when NOT using --cuda_ray)")
    parser.add_argument('--grid_size', type=int, default=128, help="density grid resolution")  # 密度网格分辨率
    parser.add_argument('--mark_untrained', action='store_true', help="mark_untrained grid")
    parser.add_argument('--dt_gamma', type=float, default=1/256, help="dt_gamma (>=0) for adaptive ray marching. set to 0 to disable, >0 to accelerate rendering (but usually with worse quality)")
    parser.add_argument('--density_thresh', type=float, default=10, help="threshold for density grid to be occupied")  # 密度网格被占用的阈值
    ...
    # batch size related
    parser.add_argument('--num_rays', type=int, default=4096, help="num rays sampled per image for each training step")  # 每张图片的光线采样数量
    parser.add_argument('--adaptive_num_rays', action='store_true', help="adaptive num rays for more efficient training")  # 自适应光线数量
    ...
    # stage 0 regularizations，第0阶段正则项
    ...
    # stage 1 regularizations，第1阶段正则项
    ...
    model = NeRFNetwork(opt)  # 根据opt参数加载模型
    
    criterion = torch.nn.MSELoss(reduction='none')  # 定义损失函数
    # criterion = torch.nn.SmoothL1Loss(reduction='none')

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  # 判断计算设备
    ...
    if opt.test:  # 测试模式
        ...
        optimizer = lambda model: torch.optim.Adam(model.get_params(opt.lr), eps=1e-15)  # 定义优化器

        train_loader = NeRFDataset(opt, device=device, type=opt.train_split).dataloader()  # 训练集数据加载器

        max_epoch = np.ceil(opt.iters / len(train_loader)).astype(np.int32)  # 根据迭代次数和训练集数据加载器长度来确定epoch
        save_interval = max(1, max_epoch // max(opt.n_ckpt, 1))  # 保存间隔
        eval_interval = max(1, max_epoch // max(opt.n_eval, 1))  # 评估间隔
        ...
        #     model.update_aabb(train_loader._data.pts_aabb)  # 更新aabb边界框
        ...
        if True:
            valid_loader = NeRFDataset(opt, device=device, type='val').dataloader()  # 验证集数据加载器

            trainer.metrics = [PSNRMeter(),]  # 训练指标
            ...
            if opt.stage == 1:
                trainer.export_stage1(resolution=opt.texture_size)  # 导出第一阶段结果
            else:
                trainer.save_mesh(resolution=opt.mcubes_reso, decimate_target=opt.decimate_target, dataset=train_loader._data if opt.mesh_visibility_culling else None)